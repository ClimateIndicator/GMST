      program globalmeannoaa
c     program to calculate global means from NOAA gridded temperature data.
c     Full documentation is in the globaltemphadcrupub.f file - this program
c     is equivalent except for data ingest
...
184   format(i5,6(f6.2))
      open(unit=1,file='avtest')
      do 185 i=1,36
       write(1,182) (obs(140,1,j,i),j=1,72)
       write(1,182) (avlandmask(j,i),j=1,72)
185   continue
      close(1)
...
c     replace ###DIRECTORY### with working directory
      open(unit=1,file=
     *'###DIRECTORY###/noaatest2022')
      do 1730 i=1,173
      write(6,1705) i
       do 1720 j=1,12
        do 1710 l=1,36
          ll=l
          read(1,*,end=1740) (obs(i,j,k,ll),k=1,72)
1710    continue
1720   continue
1730  continue
1740  continue
      ...
      do 2430 i=1,72
       do 2420 j=1,36
        ...
        do 2410 k=1,5
         do 2400 l=1,5
          lonpos=180+i*5+k-5
          if (lonpos.gt.360) then
           lonpos=lonpos-360
          endif
          sum=sum+landmask(lonpos,(j*5+l-5))
2400     continue
2410    continue
...
2440  format(72(f4.1))
      ...
      write(1,2440) ((avlandmask(k,l),k=1,72),l=1,36)
      close(1)
      return
      end
