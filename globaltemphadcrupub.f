      program globalmeanhadcrut2022
c     program to calculate global, hemispheric and land/ocean mean
c     temperatures from HadCRUTv5 grids
...
28    continue
29    format(f6.2)
c     calls routines to ingest data and land/ocean masks
      call obsingest(obs)
      call landmaskingest(landmask)
c     calls subroutine to average land mask to HadCRUT grid resolution
      call landmaskav(landmask,avlandmask)
c     calls subroutine to calculate annual grids from monthly values
      call anngridcalc(obs,annobs)
c     calls subroutine to calculate monthly and annual spatial means
...
c     section to calculate trends at each gridpoint
      do 100 j=1,72
       do 90 k=1,36
c       extracts gridpoint time series
        do 40 l=1,length
         obsline(l)=annobs(l,j,k)
40      continue
c       checks that time series is complete
        call compcheck(obsline,trendlim,trendlen,compflag)
      if (compflag.eq.1) then
c      calculates regression variables and residuals for time series, and
c      autocorrelation
...
c      assesses significance of trends       
...
c      inserts missing data codes where required
       do 50 m=1,3
...
161   format(72i4)
c     writes output files for gridpoint trends. Trendvals is central
c     estimate, trendvalsext also gives 95% confidence limits, sigmat flags
c     significant/non-significant values
...
c     writing of output files for spatial averages
c     monthly spatial means
...
c     subroutine for linear regression values for a time series
      integer yearvals(200)
      real obs(200)
      integer trendlim(2)
      real regvals(2)
...
c     subroutine to calculate residuals in regression time series
      integer yearvals(200)
      real obs(200)
      real regvals(2)
      real residuals(200)
...
c     subroutine to calculate autocorrelation of residuals in time series
      real residuals(200)
      integer trendlim(2)
      real corrval
      real corrmat(200,2)
...
c     subroutine to calculate confidence interval limits in regression
c     parameters
      real residuals(200)
      integer trendlim(2)
      real efflen
...
c     subroutine to check for sufficient data completeness. Checks both
c     full time series, and first and last 10% of time series
      real obsline(200)
      integer trendlim(2)
      integer trendlen
      integer compflag
...
1600  continue
1610  format(4i6)
c     for valid trend at a gridpoint, requires data for at least 70% of the 
c     timespan, including at least 20% of both the first and last 10% of years
...
c     ingesting the gridded temperature data
      real obs(200,12,72,36)
c     initialising matrix
      do 1703 i=1,200
       do 1702 j=1,12
        do 1701 k=1,72
         do 1700 l=1,36
...
1703  continue
1705  format(i3)
c     substitute working directory name for '###DIRECTORY###'
c     hadcrutest2022 is input file name. This is a text write of 
c     netcdf file (using ncdump and removing non-temperature lines)
c     to get around problems with this system handling NetCDF 4 file
      open(unit=1,file=
     *'###DIRECTORY###/hadcrutest2022')
      do 1730 i=1,173
      write(6,1705) i
       do 1720 j=1,12
        do 1710 l=1,36
...
c     subrotuine to calculate annual gridded values from monthly values
      real obs(200,12,72,36)
      real annobs(200,72,36)
      real obssum
      integer obscount
...
c     calculation of spatial means from grids
      real obs(200,12,72,36)
      real avlandmask(72,36)
      integer length
      real spatmean(200,12,3,3)
      ...
      do 2050 i=1,length
       do 2040 j=1,12
c       in these arrays, m is hemisphere (1 southern, 2 northern) and n
c       is a land/ocean flag (1 all, 2 land, 3 ocean)
        do 2001 n=1,3
        do 2000 m=1,2
         spatsum(m,n)=0.0
         count(m,n)=0
...
c        gridboxes weighted by area
         weight=abs((sin(lat(1))-sin(lat(2))))
         do 2020 k=1,72
          if (obs(i,j,k,l).gt.-999) then
c        separating into northern and southern hemisphere
           if (lat(1).lt.0) then
c        spatsum is sum of weighted spatial means, wsum is sum of weights
...
2020     continue
2030    continue
c       calculation of spatial means from the weighted gridbox values
        do 2036 n=1,3
        do 2035 m=1,2
...
2035    continue
c       global mean calculated as mean of NH and SH values
...
c     calculation of annual spatial means from monthly values
      real spatmean(200,12,3,3)
      real annspatmean(200,3,3)
      real sum
      integer count
...
c     calculation of annual means at each gridpoint for a set period (can
c     be confligured). Not used in core output, is a diagnostic 
      real annobs(200,72,36)
      real gridmean(72,36)
      real sum
      integer count
...
c     ingestion of land/ocean mask (from Berkeley Earth)
      real landmask(360,180)
      do 2301 i=1,360
       do 2300 j=1,180
        landmask(i,j)=-999.0
...
c     averaging of land/ocean mask to 5-degree gridboxes
      real landmask(360,180)
      real avlandmask(72,36)
      real sum
      do 2430 i=1,72
...
2400     continue
2410    continue
        avlandmask(i,j)=sum/25.0
2420   continue
2430  continue
      open(unit=1,file='avlandmask')
2440  format(72(f5.2))
      write(1,2440) ((avlandmask(i,j),i=1,72),j=1,36)
      close(1)
      return
      end
