      program globalmeanberk
c     program to calculate global mean temperatures from Berkeley Earth
c     land/ocean grids
c     see HadCRUT code (globaltemphadcrupub.f) for full documentation - this
c     code differs only in data ingest and grid resolution
      integer yearvals(200)
      real annobs(200,360,180)
      real obs(200,12,360,180)
      real obsline(200)
      real trendvals(360,180,3)
      integer sigmat(360,180)
      real spatmean(200,12,3,3)
      real annspatmean(200,3,3)
      real gridmean(360,180)
...
29    format(f6.2)
      ...
      call spatmeancalc(obs,length,spatmean,gridcount,landmask)
      call annspatcalc(spatmean,annspatmean)
c     trend calculations not implemented in this version
c      call gridmeancalc(annobs,gridmean)
c      do 100 j=1,72
c       do 90 k=1,36
c        do 40 l=1,length
c         obsline(l)=annobs(l,j,k)
c40      continue
c        call compcheck(obsline,trendlim,trendlen,compflag)
c      if (compflag.eq.1) then
c       call regcalc(yearvals,obsline,trendlim,regvals)
c       call rescalc(yearvals,obsline,regvals,residuals,trendlim)
c       call corrcalc(residuals,trendlim,corrval)
c       efflen=trendlen*(1-corrval)/(1+corrval)
c       call errcalc(residuals,trendlim,efflen,yearvals,errval)
c       trendvals(j,k,1)=regvals(2)*10.0
c       trendvals(j,k,2)=(regvals(2)-errval*1.645)*10.0
c       trendvals(j,k,3)=(regvals(2)+errval*1.645)*10.0
c        if ((trendvals(j,k,2)*trendvals(j,k,3)).ge.0) then
c         sigmat(j,k)=1
c        else
c         sigmat(j,k)=0
c        endif 
...
c90    continue
c100   continue
160    format(72(1x,f8.4,1x))
161   format(72i4)
c      open(unit=1,file='trendvals')
c      open(unit=2,file='trendvalsext')
c      open(unit=3,file='sigmat')
c      do 170 k=1,3
c       do 165 j=1,36
c        write(2,160) (trendvals(i,j,k),i=1,72)
c        if (k.eq.1) then
c         write(1,160) (trendvals(i,j,k),i=1,72)
c         write(3,161) (sigmat(i,j),i=1,72)
c        endif
c165     continue
c170    continue
c      close(3)
...
220   continue
      ...
      write(1,182) ((gridmean(j,k),j=1,360),k=1,180)
      close(1)
...
1703  continue
1705  format(i3)
c     substitute working directory for ###DIRECTORY###
      open(unit=1,file=
     *'###DIRECTORY###/berktest2022')
      do 1730 i=1,173
      write(6,1705) i
       do 1720 j=1,12
        do 1710 l=1,180
          read(1,*,end=1740) (obs(i,j,k,l),k=1,360)
1710    continue
1720   continue
1730  continue
1740  continue
...
1950  continue
      ...
      subroutine spatmeancalc(obs,length,spatmean,gridcount,landmask)
      real obs(200,12,360,180)
      integer length
      real spatmean(200,12,3,3)
      integer gridcount(200,12,3,3)
...
2005  format(2(f6.2))
        do 2030 l=1,180
         lat(1)=(l-91.0)*pi/180.0
         lat(2)=(l-90.0)*pi/180.0
